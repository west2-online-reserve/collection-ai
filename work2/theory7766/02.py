...
def main():
    ...
    # net.load_state_dict(torch.load('CIFAR100_gru1.pth'))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    net.to(device)
    loss = nn.CrossEntropyLoss()
    # updater = torch.optim.Adam(net.parameters(),lr=0.00005)
    updater = torch.optim.SGD(net.parameters(), lr=0.001)
    # 读取数据集
    batch_size = 64
    ...
    torch.save(net.state_dict(), 'CIFAR100_gru2.pth')
    tool.show(train_acc_list, test_acc_list, train_loss_list, test_loss_list)
