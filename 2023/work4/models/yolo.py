...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if nc and nc != self.yaml['nc']:  # 1. no nc: do not operator 2.nc: compare with 'nc' in yaml
...
def parse_model(d, ch):  # model_dict, input_channels(3): [3]
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from:-1, number:1, module:'Conv', args:[64,6,2,2]
        m = eval(m) if isinstance(m, str) else m  # eval strings: 将‘Conv'字符串转化为对应的类
        for j, a in enumerate(args):  # j: index, a: value in the list
            try:
                args[j] = eval(a) if isinstance(a, str) else a  # eval strings
            except NameError:
                pass

        n = n_ = max(round(n * gd), 1) if n > 1 else n  # depth gain / 1: 1, >1: n * gd / round:give max int
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3STR, C3SPP, C3Ghost, ASPP, CBAM, nn.ConvTranspose2d]:
            c1, c2 = ch[f], args[0]  # [-1]: get the last element in the list
            if c2 != no:  # if not output / no: the last output number
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]  # concat: [c_in, c_out, kernel_size, stride, padding]
            ...
            c2 = ch[f]
        # 判断是否多层 m:name, args: arguments
        # *作用在形参上，代表这个位置接收任意多个非关键字参数，转化成元组方式；*作用在实参上，代表的是将输入迭代器拆成一个个元素。
