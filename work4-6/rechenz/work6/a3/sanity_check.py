...
def generate_outputs(model, source, target, vocab):
    ...
    target_padded = model.vocab.tgt.to_input_tensor(
        target, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))

    # Run the model forward
    with torch.no_grad():
        enc_hiddens, dec_init_state = model.encode(
            source_padded, source_lengths)
        enc_masks = model.generate_sent_masks(enc_hiddens, source_lengths)
        combined_outputs = model.decode(
...
def question_1d_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    source_padded = model.vocab.src.to_input_tensor(
        src_sents, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
    ...
    assert (enc_hiddens_target.shape == enc_hiddens_pred.shape), "enc_hiddens shape is incorrect: it should be:\n {} but is:\n{}".format(
        enc_hiddens_target.shape, enc_hiddens_pred.shape)
    ...
    assert (dec_init_state_target[1].shape == dec_init_state_pred[1].shape), "dec_init_state[1] shape is incorrect: it should be:\n {} but is:\n{}".format(
        dec_init_state_target[1].shape, dec_init_state_pred[1].shape)
...
def question_1f_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    # Run Tests
    with torch.no_grad():
        dec_state_pred, o_t_pred, e_t_pred = model.step(
            Ybar_t, dec_init_state, enc_hiddens, enc_hiddens_proj, enc_masks)
    assert (dec_state_target[0].shape == dec_state_pred[0].shape), "decoder_state[0] shape is incorrect: it should be:\n {} but is:\n{}".format(
        dec_state_target[0].shape, dec_state_pred[0].shape)
    ...
    assert (dec_state_target[1].shape == dec_state_pred[1].shape), "decoder_state[1] shape is incorrect: it should be:\n {} but is:\n{}".format(
        dec_state_target[1].shape, dec_state_pred[1].shape)
...
def main():
    ...
    assert (torch.__version__ >= "1.6.0"), "Please update your installation of PyTorch >= 1.6.0. You have version {}.".format(
        torch.__version__)
    ...
    train_data_src = autograder_read_corpus(
        './sanity_check_en_es_data/train_sanity_check.es', 'src')
    train_data_tgt = autograder_read_corpus(
