...
def compute_corpus_level_bleu_score(references: List[List[str]], hypotheses: List[Hypothesis]) -> float:
    ...
    detokened_refs = [''.join(pieces).replace('▁', ' ')
                      for pieces in references]
    detokened_hyps = [''.join(hyp.value).replace('▁', ' ')
...
def train(args: Dict):
    ...
    train_data_src = read_corpus(
        args['--train-src'], source='src', vocab_size=21000)       # EDIT: NEW VOCAB SIZE
    train_data_tgt = read_corpus(
        args['--train-tgt'], source='tgt', vocab_size=8000)

    dev_data_src = read_corpus(
        args['--dev-src'], source='src', vocab_size=3000)
    dev_data_tgt = read_corpus(
        args['--dev-tgt'], source='tgt', vocab_size=2000)
        ...
        for src_sents, tgt_sents in batch_iter(train_data, batch_size=train_batch_size, shuffle=True):
            ...
            if train_iter % log_every == 0:
                writer.add_scalar("loss/train", report_loss /
                                  report_tgt_words, train_iter)
                writer.add_scalar(
                    "perplexity/train", math.exp(report_loss / report_tgt_words), train_iter)
            ...
            if train_iter % valid_niter == 0:
                writer.add_scalar("loss/val", cum_loss /
                                  cum_tgt_words, train_iter)
                ...
                writer.add_text(
                    ...
                    format_example_sentence(
                        example_sentence_src, example_sentence_tgt, example_hypothesis_beam, train_iter),
                    train_iter
                )
...
def decode(args: Dict[str, str]):
    ...
    test_data_src = read_corpus(
        args['TEST_SOURCE_FILE'], source='src', vocab_size=3000)
