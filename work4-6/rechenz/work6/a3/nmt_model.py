...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(
            in_channels=embed_size, out_channels=embed_size, kernel_size=2, padding='same')
        self.encoder = nn.LSTM(
            input_size=embed_size, hidden_size=hidden_size, bidirectional=True, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        X = self.post_embed_cnn(X.permute(1, 2, 0))  # type: ignore
        X = X.permute(2, 0, 1)
        X = torch.nn.utils.rnn.pack_padded_sequence(X, source_lengths)
        enc_hiddens, dec_init_state = self.encoder(X)  # type: ignore
        enc_hiddens, _ = torch.nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        dec_init_state = (self.h_projection(torch.cat((dec_init_state[0][0], dec_init_state[0][1]), dim=1)),  # type: ignore
                          self.c_projection(torch.cat((dec_init_state[1][0], dec_init_state[1][1]), dim=1)))  # type: ignore
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_state = self.decoder(Ybar_t, dec_state)  # type:ignore
        ...
        V_t = self.combined_output_projection(U_t)  # type: ignore
        O_t = torch.tanh(V_t)
        O_t = self.dropout(O_t)  # type: ignore
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70) -> List[
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            prev_hyp_ids = torch.div(top_cand_hyp_pos, len(
                self.vocab.tgt), rounding_mode='floor')
    ...
    def load(model_path: str):
        ...
        """
        params = torch.load(
            model_path, weights_only=False)  # 添加 weights_only=False
        args = params['args']
