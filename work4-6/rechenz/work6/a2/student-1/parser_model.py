...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # self.embed_to_hidden_weight = nn.init.xavier_uniform_(
        #     nn.Parameter(torch.tensor(embeddings)))
        # self.embed_to_hidden_bias = nn.init.uniform_(
        #     nn.Parameter(torch.tensor(embeddings)))
        # self.dropout = nn.Dropout(p=dropout_prob)
        # self.hidden_to_logits_weight = nn.init.xavier_uniform_(
        #     nn.Parameter(torch.tensor(embeddings)))
        # self.hidden_to_logits_bias = nn.init.uniform_(
        #     nn.Parameter(torch.tensor(embeddings)))
        self.embed_to_hidden_weight = nn.init.xavier_uniform_(
            nn.Parameter(torch.empty(self.embed_size*self.n_features, self.hidden_size, dtype=torch.float32)))
        self.embed_to_hidden_bias = nn.init.uniform_(
            nn.Parameter(torch.empty(self.hidden_size, dtype=torch.float32)))
        self.dropout = nn.Dropout(p=dropout_prob)
        self.hidden_to_logits_weight = nn.init.xavier_uniform_(
            nn.Parameter(torch.empty(self.hidden_size, self.n_classes, dtype=torch.float32)))
        self.hidden_to_logits_bias = nn.init.uniform_(
            nn.Parameter(torch.empty(self.n_classes, dtype=torch.float32)))
    ...
    def forward(self, w):
        ...
        logits = None
        logits = torch.matmul(x, self.embed_to_hidden_weight)
        logits = logits + self.embed_to_hidden_bias
        logits = torch.relu(logits)
        logits = self.dropout(logits)  # type: ignore
        logits = torch.matmul(logits, self.hidden_to_logits_weight)
