...
FILE = Path(__file__).resolve() # get the absolute path:(D:\Github\tph-yolov5\detect.py）
ROOT = FILE.parents[0]  # YOLOv5 root directory: (D:\Github\tph-yolov5)
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    is_file = Path(source).suffix[1:] in (IMG_FORMATS + VID_FORMATS) # suffix[1:] -> jpg
    is_url = source.lower().startswith(('rtsp://', 'rtmp://', 'http://', 'https://'))
    webcam = source.isnumeric() or source.endswith('.txt') or (is_url and not is_file) # 摄像头/.txt/网络流
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'tph_100.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / 'data/images/val', help='file/dir/URL/glob, 0 for webcam')
