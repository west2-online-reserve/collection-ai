...
def main_battle_loop():
    ...
    if action == 'attack':
        ...
        if random.random() < 0.5:  # 50%成功率
            ...
            nabiya_hp -= actual_damage
            print(f"守护之力对娜比娅造成了 {actual_damage} 点伤害！")
        else:
            print("唔…失手了…")
        nabiya_defense_bonus = 0  # 防御效果只持续一回合  
        # 4. 检查娜比娅是否被击败
    ...
    if action == 'attack':
        ...
        nagato_defense_bonus = 0  # 防御效果只持续一回合
        
    else: # defend
        print("娜比娅进入防御姿态！")
        nabiya_defense_bonus = calculate_defense_value(NABIYA_DEFEND_DICE)
        print(f"娜比娅获得了 {nabiya_defense_bonus} 点防御加成")
        # 6. 检查长门是否被击败
        # if nagato_hp <= 0:
        #     ...
