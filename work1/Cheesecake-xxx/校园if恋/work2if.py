...
class Character:
    ...
    def talk(self,time1):
        ...
        print("%s:%s"%(self.name,DIALOGUES[self.name][time1]["text"]))
        print("A:%s"%(DIALOGUES[self.name][time1]["optionA"]))
        print("B:%s" % (DIALOGUES[self.name][time1]["optionB"]))
        choice1=input("请选择你的回复(A或B，按别的键默认选B):")
        if choice1=='A':
    ...
    def give_gift(self, gift):
        ...
        add=GIFT_EFFECTS[gift][self.name]
        self.change_affinity(add)
...
class Game:
    ...
    def scene_jiejie(self):
        ...
        choice = int(input("1. 缓缓低眉，毫不在意的开始解释\n2. 头也不抬，保持敲代码的状态\n请选择："))
        # TODO 两种选择 如果选择了1 则进入该位角色的故事线 并返回 True 如果选择了 2 则进入下一位角色的选择 并且返回False
        if choice==1:
            print("你将代码结构以及涉及的算法和主要思路剖析详细地讲解给姐姐。")
            print("姐姐的眼里流露出崇拜的神情，并拉着你展开了后续的交流。你进入【姐姐】线！")
    ...
    def story_loop(self):
        ...
        while True:
            ...
            if choice==1:
                if time1 >= len(DIALOGUES[self.current_target.name]):
                    print("你和%s的对话线已经结束了，请选择别的行为"%(self.current_target.name))
                else:
                    print("你展开了和她的对话：")
                    self.current_target.talk(time1)
                    time1 += 1
            # 输入2----
            elif choice==2:
                print("有以下礼物:\nA：鲜花 B：编程笔记 C：奶茶 D：奇怪的石头 E：精致的钢笔 F：可爱玩偶 ")
                choice2=input("请选择你想要送给%s的礼物:"%(self.current_target.name))
                if choice2=='A':
                    gift='鲜花'
                elif choice2=='B':
                    gift='编程笔记'
            ...
            # 输入3----
                ...
                print("%s的好感度为%d"%(self.current_target.name,self.current_target.affinity))
