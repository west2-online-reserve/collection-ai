...
class Character:
    ...
    def give_gift(self, gift):
        ...
        gift_exist=GIFT_EFFECTS.get(gift)
        if gift_exist:
...
class Game:
    ...
    def scene_xiaobai(self):
        ...
        # 写法可以借鉴学姐线
        if choice == "1":
            print("\n你认真的给小白讲解了这道题，小白被你独特的思路迷住了")
            print("小白投来崇拜的眼光^V^,你进入了【小白】线")
    ...
    def scene_jiejie(self):
        ...
        # 要求同上
        if choice == "1":
            print("\n你向姐姐耐心的解释这代码，姐姐对你的表现很满意，你们交谈甚欢")
            print("你进入【姐姐线】！")
            self.current_target = self.characters["姐姐"]
            self.story_loop()
            return True
        else:
            print("你仍然敲着代码，姐姐失望的离开了。")
            return False
    ...
    def story_loop(self):
        ...
        while True:
            ...
            # 输入2----
                ...
                gift=input("请选择你的礼物: 鲜花/编程笔记/奶茶/奇怪的石头/精致的钢笔/可爱玩偶/夜宵外卖\n")
                self.current_target.give_gift(gift)
            # 输入3----
            elif choice == "3":
                print(f"{self.current_target.name}的好感度:{self.current_target.affinity}")
