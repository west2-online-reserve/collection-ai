...
class ModifierHandler:
    namespace: str = "builtin.modifier"
    order: int = 50

    def can_handle(self, effect: EffectSpec, ctx: ActionContext) -> bool:  # noqa: D401
        return _normalise_type(effect) in {"modifier", "stat_modifier", "modify"}

    def handle(self, effect: EffectSpec, ctx: ActionContext) -> None:  # noqa: D401
        """
        Modify scratch values that affect subsequent effect handlers.

        Supports:
        - target: "scratch" (default), "actor", "defender"/"target"
        - stat: name of the stat to modify (e.g., "dodge", "attack")
        - value: numeric value for the modification
        - operation:
            - "add"/"flat" (default): stores in {stat}_bonus, applied as `stat_final = stat_base + bonus`
            - "multiply"/"ratio": stores in {stat}_ratio, applied as `stat_final = stat_base * ratio`
            - "set"/"override": stores in {stat}_override, applied as `stat_final = override`
        """
        scratch = ctx.setdefault("scratch", {})

        # Parse parameters
        target_spec = str(effect.get("target", "scratch")).lower()
        stat = str(effect.get("stat", "")).strip()
        if not stat:
            return

        value = float(effect.get("value", 0.0))
        operation = str(effect.get("operation", "add")).lower()

        # Determine scratch key prefix based on target
        if target_spec in ("defender", "target"):
            prefix = "defender_"
        elif target_spec == "actor":
            prefix = "actor_"
        else:  # "scratch" or other
            prefix = ""

        # Store with different keys based on operation type
        if operation in ("multiply", "ratio"):
            key = f"{prefix}{stat}_ratio"
            current = float(scratch.get(key, 1.0))  # Default to 1.0 for multiply
            scratch[key] = current * value
        elif operation in ("set", "override"):
            key = f"{prefix}{stat}_override"
            scratch[key] = value
        else:  # "add" or "flat" (default)
            key = f"{prefix}{stat}_bonus"
            current = float(scratch.get(key, 0.0))
            scratch[key] = current + value
...
class DamageHandler:
    ...
    def handle(self, effect: EffectSpec, ctx: ActionContext) -> None:  # noqa: D401
        ...
        # Apply defender dodge modifiers
        dodge = float(defender.dodge)
        if "defender_dodge_override" in scratch:
            dodge = float(scratch["defender_dodge_override"])
        else:
            dodge *= float(scratch.get("defender_dodge_ratio", 1.0))
            dodge += float(scratch.get("defender_dodge_bonus", 0.0))
        dodge = max(0.0, min(1.0, dodge))  # Clamp dodge to [0, 1]

        # Apply dodge to final hit chance
        final_chance = max(0.0, chance * (1.0 - dodge))
        ...
        if not rng.chance(final_chance):
            ...
            tags.add("miss")
            ctx["abort"] = True  # Prevent subsequent effects from executing
            return
