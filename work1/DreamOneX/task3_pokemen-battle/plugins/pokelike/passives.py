...
def register_passives(bus, tm):
    ...
    def water_mitigation(log=None, ops=None, player_actor=None, enemy_actor=None, **_):
        """水系被动：受击时减少20%伤害"""
        if not ops:
            return
        combatants = []
        ...
        for creature in combatants:
            if creature and creature.element == "water" and creature.is_alive():
                # 通过 ops.grant_shield 的 mode="ratio" 实现百分比减伤
                # ops._apply_shields 会在 apply_damage 中自动处理护盾的减伤计算
                ops.grant_shield(
                    creature,
                    reduction=0.2,
                    mode="ratio",
                ...
                if log:
                    log(f"{creature.name} 水系被动：受到的伤害减少 20%。")

    bus.subscribe(Event.TURN_START, water_mitigation)
