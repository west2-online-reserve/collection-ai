...
def calculate_attack_damage(num_dice):
    ...
    return attack_points if (attack_points := roll_dice(num_dice)) < CRITICAL_HIT_THRESHOLD else attack_points * CRITICAL_HIT_SCALE
...
def main_battle_loop():
    ...
    while True:
        ...
        if action == 'attack':
            nabiya_hp -= calculate_attack_damage(NABIYA_ATTACK_DICE) - nabiya_defense_bonus
            nabiya_defense_bonus = 0
        elif action == 'defend':
            nagato_defense_bonus = calculate_defense_value(NAGATO_DEFEND_DICE)
            nabiya_defense_bonus = 0
        else: # special
            nabiya_hp -= random.choice([0, SPECIAL_ATTACK_DAMAGE])
            nabiya_defense_bonus = 0
        ...
        if action == 'attack':
            nagato_hp -= calculate_attack_damage(NAGATO_ATTACK_DICE) - nagato_defense_bonus
            nagato_defense_bonus = 0
        else:
