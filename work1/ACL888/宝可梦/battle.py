...
class Play:
    ...
    def player_choose_pokemon_team(self, pokemon_to_choose: list, num=3):
        ...
        while len(self.player_team) < num:
            ...
            if valid_choice(choice, len(pokemon_to_choose)):
                # 追加所选宝可梦的深拷贝，保证队伍中的每个 Pokemon 为独立实例
    ...
    def computer_choose_pokemon_team(self, pokemon_to_choose: list, num=3):
        ...
        # 随机选择条目，但将每个被选中的 Pokemon 深拷贝后加入电脑队伍，
        # 这样即便玩家选了同一个条目，双方也不会共享同一实例
        chosen = random.sample(pokemon_to_choose, num)
        for p in chosen:
            self.computer_team.append(copy.deepcopy(p))
    ...
    def player_use_skills(self):
        ...
        # 如果被麻痹或处于蓄力状态则无法行动
        if getattr(self.current_player_pokemon, "paralyzed", False):
            print(f"{self.current_player_pokemon.name} is paralyzed and cannot act this turn.")
            return
        if getattr(self.current_player_pokemon, "charging", False):
            print(f"{self.current_player_pokemon.name} is charging and skips this turn.")
            return

        # 使用状态效果（例如 ChargeEffect）来管理蓄力并在到期时释放；不再使用额外的 charged_ready/chared_action 机制
    ...
    def computer_use_skills(self):
        # 电脑随机选择技能
        # 如果被麻痹或处于蓄力状态则无法行动
        if getattr(self.current_computer_pokemon, "paralyzed", False):
            print(f"{self.current_computer_pokemon.name} is paralyzed and cannot act this turn.")
            return
        if getattr(self.current_computer_pokemon, "charging", False):
            print(f"{self.current_computer_pokemon.name} is charging and skips this turn.")
            return
    ...
    def battle_round(self):
        ...
        self.check_game_status()
        # 回合结束：处理状态持续时间递减与到期处理
        # 需要在双方行动后调用各自的 end_turn，以便状态效果（例如 Charge）能正确减少持续时间并在到期时触发 on_expire
        try:
            self.current_player_pokemon.end_turn()
        except Exception:
            pass
        try:
            self.current_computer_pokemon.end_turn()
        except Exception:
            pass
