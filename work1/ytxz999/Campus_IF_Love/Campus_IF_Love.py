...
class Character:
    ...
    def talk(self):
        ...
        if dialogue_pool:
            ...
            print("C: 什么都不说，只是看着她")
            Option = input("请输入你的选项").strip().upper()
            if Option == "A":
                ...
                Option1 = input("请输入你的选项").strip().upper()
                if Option1 == "A":
                    self.change_affinity(5)
                elif Option1 == "B":
                    self.change_affinity(-5)
    ...
    def give_gift(self, gift):
        ...
        gift_effect = GIFT_EFFECTS.get(gift, {})  # 先按礼物名取字典
        value = gift_effect.get(self.name, gift_effect.get("default", 0))
        if value > 0:
            reactions = [
                "哇，谢谢你！我很喜欢这份礼物～",
                "太惊喜了！这正是我想要的！",
...
class Game:
    ...
    def scene_xiaobai(self):
        ...
        #写法可以借鉴学姐线
        if choice == "1":
            print("看完题目不到十秒,你直接说出了这题的使用的数据结构和算法")
            print("小白一脸震惊，看你的眼神充满了崇拜。你进入了【小白线】")
    ...
    def scene_jiejie(self):
        ...
        #要求同上
        if choice == "1":
            print("看着姐姐，你缓缓站起身，口若悬河地说出自己的想法")
            print("姐姐听完你的话点了点头，嘴角露出微微的笑容。你进入了【姐姐线】")
            self.current_target = self.characters["姐姐"]
            self.story_loop()
            return True
        else:
            print("妈妈不让我和陌生人说话（快速走开）。")
            return False
