    ...
    while nagato_hp > 0 and nabiya_hp > 0:
        ...
        if action == "attack":
            ...
            if check_critical_hit(attack_damage):
                print("触发暴击")
                attack_damage *= 2
            attack_damage -= nabiya_defense_bonus
            if attack_damage < 0:
                attack_damage = 0
            nabiya_hp -= attack_damage
            nabiya_defense_bonus = 0
            print(f"长门对娜比亚造成了{attack_damage}点伤害")

        elif action == "defend":
            print("长门防御")
            nagato_defense_bonus = calculate_defense_value(NAGATO_DEFEND_DICE)
            print(f"长门获得了{nagato_defense_bonus}点护盾")

        else:  # special
            print("长门发动特殊攻击")
            if random.randint(1, 2) == 1:
                print("判定成功")
                attack_damage = SPECIAL_ATTACK_DAMAGE - nabiya_defense_bonus
                if attack_damage < 0:
                    attack_damage = 0
                nabiya_hp -= attack_damage
                nabiya_defense_bonus = 0
        ...
        if action == "attack":
            ...
            attack_damage -= nagato_defense_bonus
            if attack_damage < 0:
                attack_damage = 0
            nagato_hp -= attack_damage
            nagato_defense_bonus = 0
            print(f"娜比亚对长门造成了{attack_damage}点伤害")
        else:
            print("娜比亚防御")
            nabiya_defense_bonus = calculate_defense_value(NABIYA_DEFEND_DICE)
            print(f"娜比亚获得{nabiya_defense_bonus}点防御")
