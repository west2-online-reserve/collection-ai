...
class Character:
    ...
    def talk(self):
        ...
        if self.name == "学姐":
            if self.affinity == 0:
                print("你是新来的吧？画画过吗？")
                print("optionA:",DIALOGUES["学姐"][0]["optionA"],"optionB:",DIALOGUES["学姐"][0]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinity <= 5:
                print("这幅画……构图还算不错，你是随便画的吗")
                print("optionA:",DIALOGUES["学姐"][1]["optionA"],"optionB:",DIALOGUES["学姐"][1]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinity <= 45:
                print("加入社团后，不能三天打鱼两天晒网")
                print("optionA:",DIALOGUES["学姐"][2]["optionA"],"optionB:",DIALOGUES["学姐"][2]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinity <=65:
                print("我平时比较严格，你会不会觉得我很不好相处")
                print("optionA:",DIALOGUES["学姐"][3]["optionA"],"optionB:",DIALOGUES["学姐"][3]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinty <=95:
                print("今天的社团活动结束了，你要不要和我一起收拾")
                print("optionA:",DIALOGUES["学姐"][4]["optionA"],"optionB2" \
                ":",DIALOGUES["学姐"][4]["optionB"])
                option = input("请输入你的选项：")
            else:
                print("好感度已满，触发特殊CG，请自行脑补")
        if self.name == "小白":
            if self.affinity == 0:
                print("Python 里的 for 循环，我总是写不对……")
                print("optionA:",DIALOGUES["小白"][0]["optionA"],"optionB:",DIALOGUES["小白"][0]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinity <= 5:
                print("你看我借了好多书，会不会显得很傻")
                print("optionA:",DIALOGUES["小白"][1]["optionA"],"optionB:",DIALOGUES["小白"][1]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinity <= 45:
                print("写代码的时候，我总喜欢喝奶茶……你呢")
                print("optionA:",DIALOGUES["小白"][2]["optionA"],"optionB:",DIALOGUES["小白"][2]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinity <=65:
                print("你会不会觉得我太依赖你了")
                print("optionA:",DIALOGUES["小白"][3]["optionA"],"optionB:",DIALOGUES["小白"][3]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinty <=95:
                print("要不要一起留在图书馆自习？我想有人陪")
                print("optionA:",DIALOGUES["小白"][4]["optionA"],"optionB:",DIALOGUES["小白"][4]["optionB"])
                option = input("请输入你的选项：")
            else:
                print("好感度已满，触发特殊CG，请自行脑补。")
        if self.name == "姐姐":
            if self.affinity == 0:
                print("你也喜欢在咖啡店看书吗？")
                print("optionA:",DIALOGUES["姐姐"][0]["optionA"],"optionB:",DIALOGUES["姐姐"][0]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinity <= 5:
                print("研究生的生活，其实没有你想象的那么光鲜。")
                print("optionA:",DIALOGUES["姐姐"][1]["optionA"],"optionB:",DIALOGUES["姐姐"][1]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinity <= 45:
                print("你觉得，什么样的人最值得依靠？")
                print("optionA:",DIALOGUES["姐姐"][2]["optionA"],"optionB:",DIALOGUES["姐姐"][2]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinity <=65:
                print("我常常一个人坐到很晚，你不会觉得孤单吗？")
                print("optionA:",DIALOGUES["姐姐"][3]["optionA"],"optionB:",DIALOGUES["姐姐"][3]["optionB"])
                option = input("请输入你的选项：")
            elif self.affinity <= 95:
                print("如果有一天，我遇到困难，你会帮我吗？")
                print("optionA:",DIALOGUES["姐姐"][4]["optionA"],"optionB:",DIALOGUES["姐姐"][4]["optionB"])
                option = input("请输入你的选项：")
            else:
                print("好感度已满，触发特殊CG，请自行脑补。")
        self.change_affinity(5)
# from Gsu : 为什么我之前会以为下面的两个字典是拿来方便复制的。。。我知道下面这个送礼物的函数写得很蠢，但是真的不想再写一遍了
    def give_gift(self, gift):
        ...
        if self.name == "学姐":
            if gift == "1":
                ...
                print(f"{self.name}好感度+10")
            elif gift == "2":
                self.change_affinity(5)
                print(f"{self.name}好感度+5")
            elif gift == "3":
                self.change_affinity(20)
                print(f"{self.name}好感度+20")
            elif gift == "4":
                self.change_affinity(-10)
                print(f"{self.name}：何意味？")
                print(f"{self.name}好感度-10")
            elif gift == "5":
                self.change_affinity(20)
                print(f"{self.name}好感度+20")
...
class Game:
    ...
    def scene_xiaobai(self):
        ...
        if choice == "1":
            print("\n你随手在小白的草稿纸上圈了几个关键词")
            print("小白眼前一亮。你进入【小白线】")
            self.current_target = self.characters["小白"]
            self.story_loop()
            return True
        else:
            print("悄悄地你走了，不带走一片云彩")
            return False
    ...
    def story_loop(self):
        ...
        while True:
            ...
            #输入2----
                ...
                print("6.可爱玩偶")
                print("7.夜宵外卖")
                choice_gift = input("请输入选项：")
                Character.give_gift(self.current_target,choice_gift)


            #输入3----
            elif choice == "3":
                print(self.current_target.affinity)
