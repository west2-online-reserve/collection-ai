...
class Game:
    ...
    def scene_xiaobai(self)-> bool:
        ...
        # 写法可以借鉴学姐线
        if choice=="1":
            print("\n你耐心地给她讲解思路，并指导她写出这道算法题的正确答案。")
            print("小白眼睛一亮，眼神里充满崇拜。你进入【小白线】！")
    ...
    def scene_jiejie(self)-> bool:
        ...
        # TODO 两种选择 如果选择了1 则进入该位角色的故事线 并返回 True 如果选择了 2 则进入下一位角色的选择 并且返回False
        if choice == "1":
            print("\n你慢条斯理地给姐姐讲述了代码的关键点，并与姐姐一起讨论起来。")
            print("姐姐微微一笑。你进入【姐姐线】！")
            self.current_target = self.characters["姐姐"]
            self.story_loop()
            return True
        else:
            print("你一言不发，继续敲着代码。姐姐转身离开。")
            return False
