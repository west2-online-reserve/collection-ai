...
def display_status(character_name, current_hp, max_hp):
    ...
    print("【%s】HP:%f/%f" % (character_name, current_hp, max_hp))
    pass
...
def choose_nagato_action(nagato_hp, nabiya_hp):
    ...
    if nagato_hp < 30 and nabiya_hp>20 :
        return 'defend'
    elif nabiya_hp <20 :
        return 'special'
    else:
...
def check_critical_hit(base_damage):
    ...
    if base_damage >= CRITICAL_HIT_THRESHOLD:return True
    else:
        return False
    pass
...
def main_battle_loop():
    ...
    # 用 if/elif/else 处理不同行动
        if action == 'attack':
            print("长门使用‘炮击’")
            n=calculate_attack_damage(4)
            nabiya_defense_bonus-=n*(check_critical_hit(n)+1)
            if nabiya_defense_bonus < 0:
                nabiya_hp+=nabiya_defense_bonus
            nabiya_defense_bonus=0
            if n > 18:
                print("暴击了，造成了爆炸伤害%d"%(n*2))
            else:
                print("造成了伤害%d" % n)
        elif action == 'defend':
            nagato_defense_bonus=calculate_defense_value(3)
            print("长门下一回合获得护盾%d点"%nagato_defense_bonus)
        else: # special
            print("长门使用了‘特殊’赌狗技能")
            n = 30*random.randint(0,1)
            nabiya_defense_bonus -= n
            if nabiya_defense_bonus < 0:
                nabiya_hp += nabiya_defense_bonus
            nabiya_defense_bonus = 0
            if n :
                print("他赌对了，造成了爆炸伤害%d"%n)
            else:
                print("他人品实在一般，一滴血没掉")
    ...
    # 5. --- 娜比娅的回合 ---
        ...
        if action == 'attack':
            print("娜比娅使用‘炮击’")
            n = calculate_attack_damage(4)
            nagato_defense_bonus -= n * (check_critical_hit(n)+1)
            ...
            print("娜比娅下一回合获得护盾%d点" % nabiya_defense_bonus)
